                            ...
                            <div class="panel-body">
                                <div class="table-responsive">
                                    ...
                                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                        ...
                                        if ($cluster == 3) {
                                            if($id_marker!=null){
                                                $query1 = "SELECT marker.kecamatan, cluster.C1,cluster.C2,cluster.C3,cluster.Hasil FROM `cluster_3` AS cluster LEFT JOIN iterasi ON cluster.id_iterasi = iterasi.id_iterasi LEFT JOIN covid ON cluster.id_covid=covid.id_covid LEFT JOIN marker ON covid.id_marker=marker.id_marker LEFT JOIN tb_jumlah_cluster ON tb_jumlah_cluster.id=iterasi.id_jumlah_cluster WHERE iterasi.iterasi = " . $iterasi . " AND tb_jumlah_cluster.jumlah_cluster = " . $cluster . " AND covid.id_marker=".$id_marker." ORDER BY cluster.Hasil";
                                            } else {
                                                $query1 = "SELECT marker.kecamatan, cluster.C1,cluster.C2,cluster.C3,cluster.Hasil FROM `cluster_3` AS cluster LEFT JOIN iterasi ON cluster.id_iterasi = iterasi.id_iterasi LEFT JOIN covid ON cluster.id_covid=covid.id_covid LEFT JOIN marker ON covid.id_marker=marker.id_marker LEFT JOIN tb_jumlah_cluster ON tb_jumlah_cluster.id=iterasi.id_jumlah_cluster WHERE iterasi.iterasi = " . $iterasi . " AND tb_jumlah_cluster.jumlah_cluster = " . $cluster." ORDER BY cluster.Hasil ";
                                            }
                                            $sql1 = mysqli_query($koneksi, $query1);
